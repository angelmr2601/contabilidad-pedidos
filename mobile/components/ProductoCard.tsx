...
export function ProductoCard({ producto, precios, onTogglePagado, onToggleEntregado, onEdit, onDuplicate, onDelete }: { producto: Producto; precios: ConfiguracionPrecios; onTogglePagado: () => void; onToggleEntregado: () => void; onEdit: () => void; onDuplicate: () => void; onDelete: () => void }) {
  ...
  return <View style={styles.card}>
    <View style={styles.between}><View style={{ flex: 1 }}><Text style={styles.eyebrow}>Producto</Text><Text style={styles.subtitle}>{producto.cliente || "Sin cliente"}</Text></View><Badge tone={producto.pagado && producto.entregado ? "success" : "warning"}>{producto.pagado && producto.entregado ? "Listo" : "Pendiente"}</Badge></View>
