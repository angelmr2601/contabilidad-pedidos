...
export function ProductoForm({ producto, precios, onChange }: { producto: Producto; precios: ConfiguracionPrecios; onChange: (producto: Producto) => void }) {
  ...
  return <View style={styles.elevatedCard}>
    ...
    {producto.tipo === "Personalizada" ? <><Text style={styles.label}>Precios manuales</Text><View style={styles.row}><Input placeholder="Coste" value={String(producto.costeManual)} keyboardType="decimal-pad" onChangeText={(valor) => set({ costeManual: Number(valor.replace(",", ".")) || 0 })} /><Input placeholder="Venta" value={String(producto.precioVentaManual)} keyboardType="decimal-pad" onChangeText={(valor) => set({ precioVentaManual: Number(valor.replace(",", ".")) || 0 })} /></View></> : null}
    ...
    {producto.parche ? <><Text style={styles.label}>Qué parche</Text><Input placeholder="Ej: Champions League" value={producto.parcheNombre} onChangeText={(parcheNombre) => set({ parcheNombre })} /></> : null}
