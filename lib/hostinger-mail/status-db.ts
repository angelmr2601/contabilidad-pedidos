...
export function webhookPayloadToRow(payload: WebhookPayload) {
  const message =
    payload.message && typeof payload.message === "object"
      ...
      : undefined;
  const messageId = asString(payload.id) || asString(message?.id);
  const mailbox = asString(message?.mailbox) || asString(message?.mailbox_id) || "default";
  const from = addressFromPayload(message);
  const timestamp =
    asString(message?.timestamp) || asString(message?.received_at) || new Date().toISOString();
  ...
  return {
    ...
    excerpt: asString(message?.excerpt || message?.snippet || message?.text),
...
function rowToSummary(row: MailStatusRow): HostingerMessageSummary {
  return {
    ...
    excerpt: row.excerpt || "Mensaje recibido por webhook. Abre Hostinger para ver el cuerpo completo hasta conectar el endpoint REST oficial.",
...
export async function listWebhookMessages({
  ...
  let request = supabase
    .from("hostinger_mail_message_statuses")
    .select("message_id, mailbox, status, sender_email, sender_name, subject, excerpt, received_at, updated_at", { count: "exact" })
    .order("received_at", { ascending: false, nullsFirst: false })
    .range(from, to);

  if (query) {
    request = request.or(`subject.ilike.%${query}%,sender_email.ilike.%${query}%,excerpt.ilike.%${query}%`);
  }
